...
def test_array_animator_wcs_2d_clip_interval_change(wcs_4d):
    data = np.arange(120).reshape((5, 4, 3, 2))
    pclims = [5, 95]
    a = ArrayAnimatorWCS(data, wcs_4d, [0, 0, "x", "y"], clip_interval=pclims * u.percent)
...
def test_array_animator_wcs_coord_params_no_ticks(wcs_4d):
    coord_params = {
        "hpln": {"format_unit": u.deg, "major_formatter": "hh:mm:ss", "axislabel": "Longitude", "ticks": False}
    }
...
def test_array_animator_wcs_coord_params_grid(wcs_4d):
    coord_params = {
        "hpln": {"format_unit": u.deg, "major_formatter": "hh:mm:ss", "axislabel": "Longitude", "grid": True}
    }
